    ...
    if (
      ...
      results = results.filter((doc) => {
        ...
        if (selectedSubjects.length > 0) {
          const q = selectedSubjects[0].toLowerCase();
          if (!doc.subject.toLowerCase().includes(q)) return false;
        }

        if (selectedTeachers.length > 0) {
          const q = selectedTeachers[0].toLowerCase();
          if (!doc.teacher.toLowerCase().includes(q)) return false;
        }
          ...
          <main className="flex-1">
            <div className="mb-4">
              <p className="text-gray-600">
                Tìm thấy{" "}
                <span className="font-semibold">{filteredDocs.length}</span> tài
                liệu
              </p>
            </div>
